export function Login(){ // เข้าสู่ระบบ
...
export function buycancer(age,Sum_insured){
...
export function Travel(Travel,Person) { //เลือกแผนประกัน ประเทศ,จำนวนคน
    cy.get('#travel_destination').then(function($select){$select.val(Travel)})
    cy.get('#travel_person').then(function($select){$select.val(Person)})
}
export function SelectMotor(Manufacturer,Model,Year,Plate){ //เลือกรถยนต์,รุ่น,ปี,จังหวัด
...
export function Motor(s){ //เลือกระดับประกัน
...
export function SetDate(type,i) { // เปลี่ยนวัน ปกติหรือวันถัดไป,จำนวนวันที่ต้องการ
    var date = new Date();
...
export function ChangeInsuran(i){ //เลือกผู้ให้ประกัน
...
export function CreditCard() { //จ่ายผ่านบัตรเครดิต
...
export function QR() { //จ่ายแบบ QRPayment
...
export function CheckStatus(){ // เช็คสเตตัสหลังบ้าน
...
export function CheckStatusOrder(OrderNumber){ //เรียกใช้การทำงาน เช็คสถานะที่หลังบ้าน 
    Credit()
    cy.wait(30000)
    IntoStaff()
    CheckOrder(OrderNumber)
}
export function IntoStaff(){ // เข้าสู่ระบบหลังบ้าน
...
export function CheckOrder(OrderNumber){ //รับเลขออเดอร์ มาเช็คหลังบ้าน
    cy.visit('http://staging.gettgo.com/staff/orders')
    cy.get('#order_order_number').type(OrderNumber)
    cy.contains("ส่ง Policy แล้ว")
}
export function FillCustomerInformations(){ //ใส่ข้อมูลส่วนตัว
...
export function genLicensePlate() { //สุ่มเลขทะเบียน
    const title = 'กข'
...
export function genVehicleModelNumber() { //สุ่มเลขตัวเครื่อง
    var modelNumber = "";
